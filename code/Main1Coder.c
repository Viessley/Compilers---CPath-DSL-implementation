...
cp_intg main1Coder(cp_intg argc, cp_strg* argv) {
	if (argc < 5) {
		printf("Usage: %s [cypher=1|decypher=0] <input_file> <output_file>\n", argv[0]);
		return EXIT_FAILURE;
	}
	cp_strg operation = "";
	cp_strg inputFileName = "";
	cp_strg key = STR_LANGNAME;
	cp_strg outputFileName = "";
	if (argc > 4) {
		operation = argv[2];
 ...
 /* Call the other operation in memory */
		cp_intg size = getSizeOfFile(outputFileName);
		cp_strg output;
		if (atoi(operation) == CYPHER)
